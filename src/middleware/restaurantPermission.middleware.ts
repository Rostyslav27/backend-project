...
export const restaurantPermissionMiddleware = (roles:RestaurantRole[], options:{self?:boolean} = {}) => {
  return async function(req:Request, res:Response, next:NextFunction):Promise<void> {
    try {
      ...
      if (req.baseUrl === '/rooms') {
        getRestaurantMethod = restaurantService.getRestaurantByRoomId(+req.params.id)
      }
      ...
      if (req.baseUrl === '/profiles') {
        getRestaurantMethod = restaurantService.getRestaurantByProfileId(+req.params.id)
      }
